...
import { BrowserRouter, Route, Routes, Navigate } from "react-router-dom"; // Importa BrowserRouter, Route, Routes e Navigate do pacote 'react-router-dom'
import { Home } from "./pages/Home"; // Importa o componente Home
import { Shop } from './pages/Shop'; // Importa o componente Shop
import { About } from './pages/About'; // Importa o componente About
import { Contact } from './pages/Contact'; // Importa o componente Contact
import { Header } from "./components/Header"; // Importa o componente Header
import { Footer } from "./components/Footer"; // Importa o componente Footer
import { ProductCrud } from "./pages/ProductCrud"; // Importa o componente ProductCrud
import { SingleProduct } from "./pages/SingleProduct"; // Importa o componente SingleProduct
...
const Router = () => {
    ...
    if (isCheckingAuthentication) {
        return <div>Verificando autenticação...</div>; // Retorna uma mensagem de carregamento enquanto verifica a autenticação
    }

    // Retorna as rotas da aplicação utilizando o BrowserRouter para envolver todas as rotas
    return (
        <BrowserRouter>
            <Header/> {/* Renderiza o componente Header, responsável pela barra de navegação */}
            <Routes>
                <Route path="/" element={<Home />} /> {/* Define a rota para a página inicial */}
                <Route path="/shop" element={<Shop />} /> {/* Define a rota para a página de loja */}
                <Route path="/about" element={<About />} /> {/* Define a rota para a página Sobre */}
                <Route path="/contact" element={<Contact />} /> {/* Define a rota para a página de contato */}
                <Route path="/product/:id" element={<SingleProduct />}></Route> {/* Define a rota para a página de produto único */}
                <Route
                    path="/productcrud"
...
/* 
    ...
    Hooks:
        O useState e o useEffect são exemplos de hooks em React. Hooks são funções especiais fornecidas pelo
        React que permitem que você use o estado e outros recursos do React em componentes de função.
        Eles permitem que você "enganche" funcionalidades adicionais em seus componentes de função,
        facilitando a separação de preocupações e a reutilização de código.
    ...
    assíncronas. São fundamentais para o desenvolvimento de aplicativos React modernos e responsivos.
*/
